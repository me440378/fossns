	...
	<el-row>
		<el-col :span="6">
			...
			<div slot="header" class="clearfix">
				...
				<div>
					<el-input placeholder="请输入举报人用户名" v-model="search.data.customer_name" :disabled="!search.useCustomerName" style="margin:10px 10px 10px 10px;">
	      		<el-checkbox label="举报人" slot="prepend" v-model="search.useCustomerName"></el-checkbox>
	    		</el-input>
	    		<el-input placeholder="请输入话题名称关键字" v-model="search.data.topic_name" :disabled="!search.useTopicName" style="margin:10px 10px 10px 10px;">
	      		<el-checkbox label="被举报话题名称" slot="prepend" v-model="search.useTopicName"></el-checkbox>
	    		</el-input>
		...
		<el-col :span="18">
			<el-card class="box-card" shadow="never">
			<div slot="header" class="clearfix">
					<span style="font-family:Microsoft YaHei;font-size: 24px;">话题举报管理</span>
				</div>
				<div>
					<el-table :data="table.items" style="width: 100%;text-align: left;">
						<el-table-column prop="customer_name" label="举报人" width="150"></el-table-column>
      			<el-table-column prop="topic_name" label="被举报话题名称"  width="400"></el-table-column>
      			<el-table-column  prop="content" label="举报原因" width="400"></el-table-column>
      		  <el-table-column  prop="created_at" label="举报时间" width="200"></el-table-column>
        ...
        <el-dialog :title="form.title" :visible.sync="form.visible" width="40%" size="tiny" >
			  <el-form ref="form" :model="form.data" label-width="150px">
			    ...
			    <el-form-item label="举报话题名称：">
			     <span style="display: block;text-align:left;font-size: 16px;font-family:Microsoft YaHei;width: 370px;" >{{form.data.topic_name}}</span>
			    </el-form-item>
...
export default{
	...
	data(){
        ...
        pageIndex: 1,
				items:[
					// {id:'1',creator_name:'limeng',content:'话题太暴力',topic_name:'小小小心',created_at:"2018-02-12",is_checking:1},
					// {id:'2',creator_name:'zhangsan',content:'话题没热度',topic_name:'小小心',created_at:"2018-03-12",is_checking:1},
					// {id:'3',creator_name:'zhangxin',content:'话题没深度',topic_name:'小小心心',created_at:"2018-04-12",is_checking:1},
				]
			},
			   form: {
			        visible:false,
    ...
    handleCheck:function(report_id,customer_id){
      ...
      key.customer_id=customer_id
 			var me=this;
       this.$post("/admin/topicreport/check",key)
			.then(res=>{
				if(res.data.error==0){
     ...
     if(this.search.useStartCreated_at&&this.search.data.startCreated_at){
      ...
      var me=this;
      this.$get("/admin/topicreport/search",key)
			.then(res=>{
